...
function SpeakerBackdrop({ active = false, featured = false, compact = false }) {
  if (!active) {
    return null;
  }

  const outerSize = featured ? 'h-48 w-48 sm:h-60 sm:w-60' : compact ? 'h-24 w-24' : 'h-36 w-36';
  const innerSize = featured ? 'h-40 w-40 sm:h-52 sm:w-52' : compact ? 'h-20 w-20' : 'h-32 w-32';

  return (
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none">
      <div
        className={`absolute rounded-full bg-white/10 ${outerSize}`}
        style={{ animation: 'speakerGlow 1.25s ease-in-out infinite' }}
      />
      <div
        className={`absolute rounded-full bg-white/25 ${innerSize}`}
      ...
      >
        {shouldShowVideo ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_top,#31445f_0%,#1f2d44_45%,#142033_100%)]">
            <SpeakerBackdrop active={isSpeaking} featured={featured} compact={compact} />
...
export default function VideoGrid({
  ...
  const standardGridTiles = useMemo(() => {
    ...
    if (count === 5) return 'grid-cols-2 md:grid-cols-3 max-w-7xl';
    return 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4 max-w-[90rem]';
